...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ─── Monthly deal-limit enforcement (server-authoritative, FAIL-OPEN) ───
    // Mirrors the client's manage-subscription 'campaign_usage' math EXACTLY so a
    // paying merchant can never be wrongly blocked: a campaign counts against the
    // limit if it is active during the current calendar month. Any missing
    // subscription/tier or query error → we ALLOW (fail open); a hard block only
    // happens when we can positively confirm the merchant is at/over their limit.
    // Uses the service role so the count isn't narrowed by RLS.
    try {
      const isDotd = is_deal_of_the_day === true;
        ...
        if (typeof limit === 'number' && limit >= 0) {
          ...
          if (typeof count === 'number' && count >= limit) {
            return new Response(JSON.stringify({
              error: isDotd
                ? "You've used all your Deal of the Day slots for this month on your current plan."
                : "You've reached your monthly deal limit for your current plan.",
            }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 403 });
          }
        }
      }
    } catch (e: any) {
      console.warn('[create-campaign] deal-limit check skipped (fail-open):', e?.message || e);
    }
    // ─── End monthly deal-limit enforcement ───
