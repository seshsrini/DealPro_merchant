...
Deno.serve(async (req) => {
  ...
  if (req.method !== 'POST') { // Use POST to receive lang in body
  ...
  try {
    ...
    await authenticateRequest(req); // Ensure only authenticated users can get states
    ...
    if (error) {
      console.error('[location/get-states EF] Supabase select failed:', error.message);
      throw error;
    }

    const states = (data || []).map((state: any) => ({
      ...state,
      display_name: state.names[lang as string] || state.names['en'] || "Unknown State"
    }));
    ...
    console.error('[location/get-states EF] Failed to fetch states:', error.message || error);
    let status = 500;
