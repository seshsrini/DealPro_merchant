...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: campaigns, error: campaignsError } = await supabase
      .from('campaigns')
      .select('campaign_id, deal_heading, offer_value, category, start_date, end_date, status')
    ...
    // Fetch redemption data (campaign_interactions) for revenue calculation
    const campaignIds = campaigns.map((c: any) => c.campaign_id);
    const { data: claims, error: claimsError } = await supabase
      .from('campaign_interactions')
      .select('campaign_id, redeemed_at, is_redeemed')
    ...
    campaigns.forEach((campaign: any) => {
      ...
      const campaignClaims = (claims || []).filter((c: any) => c.campaign_id === campaign.campaign_id && c.is_redeemed === true);
      ...
      // Parse "30% OFF" or "Flat ₹200 Off" etc — extract first number
      const discountMatch = (campaign.offer_value || '').match(/(\d+)/);
      const discountPercent = discountMatch ? parseFloat(discountMatch[1]) : 15;
