...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // paying merchant can never be wrongly blocked. Fail-open on any missing
    // subscription/tier or error; hard-block only when positively over the limit.
    try {
      ...
      console.warn('[create-campaign-fast] deal-limit check skipped (fail-open):', e?.message || e);
    }
