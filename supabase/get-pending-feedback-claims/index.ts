...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[redemption/get-pending-feedback-claims EF] Authenticated user ID: ${user.id}`);
    ...
    if (interactionsError) {
      console.error('[redemption/get-pending-feedback-claims EF] Supabase select (interactions) failed:', interactionsError.message);
      throw interactionsError;
    }
    console.log(`[redemption/get-pending-feedback-claims EF] Raw interactions fetched: ${interactions?.length || 0} records.`);
    ...
    if (ratingsError) {
      console.error('[redemption/get-pending-feedback-claims EF] Supabase select (ratings) failed:', ratingsError.message);
      throw ratingsError;
    }
    console.log(`[redemption/get-pending-feedback-claims EF] Raw ratings fetched: ${ratings?.length || 0} records.`);

    const ratedIds = new Set(ratings?.map(r => r.campaign_id) || []);
    
    // Filter out interactions that already have a rating
    const filteredClaims = (interactions || [])
      ...
      .map((i: any) => {
        ...
        const campaignDetails = {
          ...
          localized_shop_name: i.campaigns?.localized_shop_name || {},
    ...
    console.log(`[redemption/get-pending-feedback-claims EF] Filtered pending feedback claims: ${filteredClaims.length} records. Sample:`, filteredClaims[0]);

    return new Response(JSON.stringify(filteredClaims), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
  } catch (error: any) {
    console.error('[redemption/get-pending-feedback-claims EF] Failed to fetch pending feedback claims:', error.message || error);
    let status = 500;
    if (error.message && typeof error.message === 'string') {
      if (error.message.includes('Unauthorized')) {
        ...
        status = 404;
        error.message = 'No pending feedback claims found for this user.';
      }
    }
