...
export const StepFullName: React.FC<StepFullNameProps> = ({ value, onChange, onNext, onBack, theme }) => {
  ...
  return (
    <div className="flex flex-col min-h-full px-6 pt-6">
      <div style={floatIn(0, visible)} className="w-16 h-16 rounded-2xl bg-slate-500/10 flex items-center justify-center mb-6">
      ...
      <h2 style={floatIn(100, visible)} className={`text-2xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
        {t('ob_name_title')} <span className="text-red-500">*</span>
      </h2>
      ...
      <div style={floatIn(300, visible)}>
        ...
        {value.length > 0 && !isValid && (
          <p className="text-xs text-red-500 mt-2">{t('ob_name_min')}</p>
        )}
      </div>
