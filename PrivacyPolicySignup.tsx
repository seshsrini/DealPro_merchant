...
import { X, Shield, Lock, Eye, Users, Database, ChevronDown } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[1000] bg-black/50">
      <div className="max-w-md mx-auto w-full h-full relative">
        ...
        >
          ...
          {/* Scroll Indicator */}
          {!hasScrolledToBottom && (
            <div className={`shrink-0 px-5 py-2 flex items-center justify-center gap-2 border-b ${isDark ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'}`}>
              <ChevronDown className={`w-3.5 h-3.5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
              <p className={`text-xs font-medium ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                Please scroll down to read the complete privacy policy
              </p>
            </div>
          )}
