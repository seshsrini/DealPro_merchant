...
/**
 * DealPro logo (inlined SVG). Inlining lets the popup window render the brand
...
const brandFooter = (d: ReturnType<typeof extract>): string => `
  <div class="brand-footer">
    <div class="brand-footer-cta">
      Get the DealPro app — discover more deals near you
    </div>
...
const renderSideBySide = (d: ReturnType<typeof extract>): string => `
  <div class="page page-side-by-side">
    ...
    <div class="sbs-details">
      ...
      <div class="sbs-cta">Get the DealPro app for more deals near you</div>
    </div>
  </div>
`;
...
<head>
  ...
  <title>${escapeHtml(d.heading || 'DealPro Poster')}</title>
...
export function openDealPoster(deal: Deal, template: PosterTemplate): boolean {
  ...
  if (Capacitor.isNativePlatform()) {
    ...
    if (printer?.print) {
      try {
        printer.print(html, { name: `DealPro Poster — ${deal && (deal as any).deal_heading ? (deal as any).deal_heading : 'Deal'}` }, () => {});
        return true;
      } catch (err) {
