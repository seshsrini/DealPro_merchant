...
Deno.serve(async (req) => {
  ...
  if (req.method !== 'POST') { // Using POST to get cityId in body
  ...
  try {
    ...
    const { cityId, lang } = await req.json();

    // 1. Validate Input Data
    if (!isPositiveNumber(cityId)) {
      return new Response(JSON.stringify({ error: 'Valid City ID is required.' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 });
    }
    ...
    if (error) {
      console.error('[location/get-localities EF] Supabase select failed:', error.message);
      throw error;
    }

    const localities = (data || []).map((loc: any) => ({
      ...loc,
      display_name: loc.names[lang as string] || loc.names['en'] || "Unknown Locality"
    }));
    ...
    console.error('[location/get-localities EF] Failed to fetch localities:', error.message || error);
    let status = 500;
