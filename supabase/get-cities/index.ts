...
Deno.serve(async (req) => {
  ...
  if (req.method !== 'POST') { // Using POST to get stateId in body
  ...
  try {
    ...
    const { stateId, lang } = await req.json();

    // 1. Validate Input Data
    if (!isPositiveNumber(stateId)) {
      return new Response(JSON.stringify({ error: 'Valid State ID is required.' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 });
    }
    // Lang validation could be added if strict
    
    const { data, error } = await supabase
      .from('cities')
      .select('id, state_id, names')
      .eq('state_id', stateId)
      .order('names->>en');
    
    if (error) {
      console.error('[location/get-cities EF] Supabase select failed:', error.message);
      throw error;
    }

    const cities = (data || []).map((city: any) => ({
      ...city,
      display_name: city.names[lang as string] || city.names['en'] || "Unknown City"
    }));
    ...
    console.error('[location/get-cities EF] Failed to fetch cities:', error.message || error);
    let status = 500;
